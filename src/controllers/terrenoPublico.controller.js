...
exports.obterPublico = async (req, res) => {
  try {
    ...
    console.log(
      `[BACKEND] /terreno-publico/${matricula}: origin_x=${terreno.origin_x} origin_y=${terreno.origin_y} ` +
        `pontos=${pontos.length} confrontantes retornados=${confrontantes.length}` +
        (confrontantes[0] ? ` primeiro=${JSON.stringify(confrontantes[0])}` : "")
    );
    ...
    return res.json({
      ...
      area: terreno.area,
      // Campo NOVO (aditivo). `area` continua sendo exatamente o mesmo
      // número de antes -- nenhum campo existente foi removido ou
      // renomeado. `null` para terrenos sem unidade cadastrada.
      area_unidade: terreno.area_unidade,
